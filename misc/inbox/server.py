...
class MyRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        self.end_headers()
        command = "ag -o --no-filename '\[\[para/([^\]]*)\]\]' notes/ ~/ProtonDrive/wiki/personal/pages/ | sort | uniq"
    ...
    def do_POST(self):
        try:
            ...
            if path.startswith('/saveNotes'):
                ...
                title = json_data['title']
                note_filename = f"notes/notes%2F{escape_title(title)}.md"
                print(f"mv {note_filename} ~/ProtonDrive/wiki/personal/pages/")
                os.system(f"mv {note_filename} ~/ProtonDrive/wiki/personal/pages/")
                response_content = b''
                status = 200
            else:
