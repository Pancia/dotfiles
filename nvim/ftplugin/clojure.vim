...
call SEMICOLON_GROUP('ll', '+linting')
call SEMICOLON_CMD('llr', 'Lsp_exe_here("resolve-macro-as", input("Resolve as?"), input("kondo config absolute path"))', 'resolve-as')

call SEMICOLON_GROUP('lr', '+refactorings')
call SEMICOLON_CMD('lrc', 'CleanNamespace()', 'clean namespace & zprint format')

call SEMICOLON_GROUP('lrt', '+threading')
call SEMICOLON_CMD('lrtf', 'Lsp_exe_here("thread-first")', 'thread-first')
call SEMICOLON_CMD('lrtt', 'Lsp_exe_here("thread-last")', 'thread-last')
call SEMICOLON_CMD('lrtF', 'Lsp_exe_here("thread-first-all")', 'thread-first-all')
call SEMICOLON_CMD('lrtT', 'Lsp_exe_here("thread-last-all")', 'thread-last-all')
call SEMICOLON_CMD('lrtu', 'Lsp_exe_here("unwind-thread")', 'unwind-thread')
call SEMICOLON_CMD('lrtU', 'Lsp_exe_here("unwind-all")', 'unwind-all')

call SEMICOLON_GROUP('lrr', '+requires')
call SEMICOLON_CMD('lrra', 'Lsp_exe_here("add-missing-libspec")', 'add-missing-libspec')

call SEMICOLON_GROUP('lrl', '+let')
call SEMICOLON_CMD('lrle', 'Lsp_exe_here("expand-let")', 'expand-let')
call SEMICOLON_CMD('lrlm', 'Lsp_exe_here("move-to-let", input("Binding name: "))', 'move-to-let')
call SEMICOLON_CMD('lrli', 'Lsp_exe_here("introduce-let", input("Binding name: "))', 'introduce-let')
