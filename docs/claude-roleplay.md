...
| Roster | `ai/roleplay/roster.tsv` | the cast — 57 characters across 6 universes |
| Hook | `bin/roleplay-roll`, wired in `rcs/claude-settings.json` | rolls, and injects the chosen character |
| Examples | `ai/roleplay/examples.md` | style reference — 235 worked bookends, 4–5 per character. **Not read at runtime** |
...
|---|---|---|
| `ai/roleplay/roster.tsv` | **next prompt** | the hook is a fresh subprocess per prompt that re-reads the file; nothing caches it |
...
1. Add a line to `ai/roleplay/roster.tsv`.
...
`ai/roleplay/examples.md` holds 235 worked bookends — four per character (two
...
`cmds test roleplay` — 37 tests under the `bin/roleplay-roll` component, registered
in `COMPONENTS` (`lib/python/run_tests.py`). They drive the script as a subprocess.
